...
if __name__ == '__main__':
    ...
    predictLabels = regressionMethod(trainData, trainLabels, testData, method=1, max_depth=9, n_estimators=30)
    predictFrame = pd.DataFrame(np.transpose([df2['datetime'], predictLabels]),
                                columns=['datetime', 'count'])
    predictFrame.to_csv('results/RF_9_30_v1.csv', sep=',', index=None)
